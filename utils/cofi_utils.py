...
from utils.minus_utils import prune_lora_layer, prune_linear_layer
    ...
    if zs is not None:
        if "intermediate_z" in zs:
            for layer in range(num_layers):
                if zs["intermediate_z"][layer] is None:
                    continue
                intermediate_z = zs["intermediate_z"][layer].squeeze().clone()
        ...
        if "head_z" in zs:
            for layer in range(num_layers):
                if zs["head_z"][layer] is None:
                    continue
                head_z = zs["head_z"][layer].squeeze().clone()
        ...
        if "hidden_z" in zs:
            hidden_z = zs["hidden_z"].squeeze().clone()
...
def prune_model_with_z(zs, model):
    ...
    if "head_z" in zs:
        ...
        for layer in range(len(head_z)):
            head_z_layer = head_z[layer].squeeze().clone()
    ...
    if "intermediate_z" in zs:
        ...
        for layer in range(len(intermediate_zs)):
            ...
            intermediate_z_layer = intermediate_z_layer.clone()
    ...
    if "hidden_z" in zs:
        ...
        bert.embeddings.word_embeddings.weight = torch.nn.parameter.Parameter(
            bert.embeddings.word_embeddings.weight.index_select(1, index).detach().clone())
        ...
        bert.embeddings.position_embeddings.weight = torch.nn.parameter.Parameter(
            bert.embeddings.position_embeddings.weight.index_select(1, index).detach().clone())
        ...
        bert.embeddings.token_type_embeddings.weight = torch.nn.parameter.Parameter(
            bert.embeddings.token_type_embeddings.weight.index_select(1, index).detach().clone())
...
def prune_intermediate_layers(model, keep_dims):
    ...
    for layer in keep_dims:
        if len(keep_dims[layer]) == 0:
            ...
            if isinstance(bert.encoder.layer[layer].intermediate.dense, lora.Linear) and bert.encoder.layer[layer].intermediate.dense.r > 0:
                bert.encoder.layer[layer].intermediate.dense = prune_lora_layer(bert.encoder.layer[layer].intermediate.dense, index=torch.LongTensor(keep_dims[layer]).to(device), dim=0)
            else:
...
def parse_cofi_zs(zs):
    binary_zs = {
        k: (v > 0.5).float() for k, v in zs.items()
    }
    binary_zs['head_z'] = binary_zs['head_z'].squeeze()
    binary_zs['intermediate_z'] = binary_zs['intermediate_z'].squeeze()
    binary_zs['head_z'] = binary_zs['head_z'].T.mul(binary_zs['head_layer_z']).T
    binary_zs['intermediate_z'] = binary_zs['intermediate_z'].T.mul(binary_zs['mlp_z']).T
    return binary_zs['head_z'], binary_zs['intermediate_z'], binary_zs['hidden_z']
