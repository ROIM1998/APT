...
from transformers import (HfArgumentParser, SquadDataTrainingArguments)
...
def main():
    ...
    data_args.task_name = 'squad_v2' if data_args.version_2_with_negative else 'squad'
    config, tokenizer, model = build_model(model_args, data_args, training_args)
    model_generative = 't5' in model.config.model_type or 'bart' in model.config.model_type or model.config.model_type == 'llama'
    train_dataset, eval_dataset, _, datasets, answer_column_name = build_squad_data(data_args, training_args, tokenizer, is_v2=data_args.version_2_with_negative, generative=model_generative)
    ...
    if 't5' in model.config.model_type:
        pruner = BetterFisherPruner(model, ['head_mask', 'intermediate_mask'], {'head_mask': scorer, 'intermediate_mask': scorer}, 170, False, ['search', 'rearrange'])
    else:
        pruner = BetterFisherPruner(model, ['head_mask', 'intermediate_mask'], {'head_mask': scorer, 'intermediate_mask': scorer}, 170, False, ['search', 'rearrange', 'rescale'])
