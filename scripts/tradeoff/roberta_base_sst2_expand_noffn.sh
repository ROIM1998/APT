...
#SBATCH --mem=40G                 # Memory per node (total memory)
...
student_param_tuning_config=q:0-11,v:0-11
lora_alpha=$(($lora_r * 2))

expanding_rates=(2 3 5 6)

for expanding_rate in ${expanding_rates[@]}; do
    output_dir="output/${model_name}/${task_name}/bz${training_batch_size}/elastictuning_virtualprune_expand${expanding_rate}/mac${mac_constraint}/epoch${num_train_epochs}/distill_epoch${distill_epoch}/numprune${num_prunings}/sparsity_warmup${sparsity_warmup_epochs}/pruning_start${pruning_start}/pruning_stop${pruning_stop}/lora_r${lora_r}/lora_alpha${lora_alpha}/warmup_param${warmup_param_tuning_config}/teacher_param${teacher_param_tuning_config}/distill_${distillation_type}/distill_mapping_${distill_mapping_strategy}"
    echo $output_dir
    mkdir -p $output_dir
    ...
    python run_minus_training.py \
        ...
        --tuning_expanding_ratio ${expanding_rate} \
        --max_lora_r $(($lora_r * $expanding_rate * 2)) \
