...
output_dir="output/${model_name}/${task_name}/bz${training_batch_size}/elastictuning_virtualprune_pre-tuning-prune${pre_tuning_constraint}/mac${mac_constraint}/epoch${num_train_epochs}/distill_epoch${distill_epoch}/numprune${num_prunings}/sparsity_warmup${sparsity_warmup_epochs}/pruning_start${pruning_start}/pruning_stop${pruning_stop}/lora_r${lora_r}/lora_alpha${lora_alpha}/warmup_param${warmup_param_tuning_config}/teacher_param${teacher_param_tuning_config}/"
echo $output_dir
mkdir -p $output_dir
