...
num_train_epochs=15
warmup_param_tuning_config=dq:0-39,dv:0-39
teacher_param_tuning_config=dq:0-39,dv:0-39
student_param_tuning_config=dq:0-39,dv:0-39


output_dir="llama_output/${model_name}/${task_name}/bz${training_batch_size}/elastictuning_virtualprune_pre-tuning-prune-kurtosis-${pre_tuning_constraint}-noexpand-nodistill/mac${mac_constraint}/epoch${num_train_epochs}/numprune${num_prunings}/sparsity_warmup${sparsity_warmup_epochs}/pruning_start${pruning_start}/pruning_stop${pruning_stop}/lora_r${lora_r}/lora_alpha${lora_alpha}/warmup_param${warmup_param_tuning_config}/teacher_param${teacher_param_tuning_config}/"
echo $output_dir
mkdir -p $output_dir
