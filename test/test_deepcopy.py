...
def main():
    sys.argv = ['test_pre_tuning_prune.py',
            ...
            '--model_name_or_path',
            'output/debug_momentum_distill/pre_pruning_model_step581',
            '--task_name',
            'sst2',
            '--do_train',
            '--do_eval',
    ...
    for i in range(12):
        model.bert.encoder.layer[i].intermediate_z = None
    for p in model.parameters():
        p.grad = None
        
    weight = model.bert.encoder.layer[0].attention.self.query.lora_A
