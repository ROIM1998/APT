...
def main():
    ...
    scorer._gather_score()
    
    outputs = model(**inputs, use_cache=False)
    outputs[0].backward()
    scorer.all_hidden_kurtosis_log
    ...
    mha_salience_mean, mha_kurtosis_mean = mha_saliences.mean(), mha_kurtosis.clamp(min=0).sqrt().mean()
    mha_salience_mean, mha_kurtosis_mean = mha_salience_mean / pruner.param_per_block['head_mask'], mha_kurtosis_mean / pruner.param_per_block['head_mask']
    
    ffn_saliences = torch.cat(scorer.all_ffn_scores)
    ffn_kurtosis = torch.cat(scorer.all_ffn_kurtosis)
    ffn_salience_mean, ffn_kurtosis_mean = ffn_saliences.mean(), ffn_kurtosis.clamp(min=0).sqrt().mean()
    ffn_salience_mean, ffn_kurtosis_mean = ffn_salience_mean / pruner.param_per_block['intermediate_mask'], ffn_kurtosis_mean / pruner.param_per_block['intermediate_mask']
    
    hidden_salience_mean = scorer.all_hidden_scores.mean()
    hidden_kurtosis_mean = scorer.all_hidden_kurtosis.mean()
    hidden_salience_mean, hidden_kurtosis_mean = hidden_salience_mean / pruner.param_per_block['hidden_mask'], hidden_kurtosis_mean / pruner.param_per_block['hidden_mask']
