...
def main():
    ...
    prefix = 'backup_' if model.virtual_pruned else ''
    mask_attrs = ['head_mask', 'intermediate_mask', 'hidden_mask']
    retained_indices = {
        k: getattr(model, prefix + k).view(-1).nonzero().squeeze()
        for k in mask_attrs
    }
    if scorer is not None:
        for k in mask_attrs:
            # Update accumulated salience and uncertainty
            scorer.set_retained_indices(k, retained_indices[k], clean_to_zero=False)
    
    # Randomly masking 40% of the model
    model.head_mask[torch.randperm(model.head_mask.shape[0])[:int(model.head_mask.shape[0] * 0.6)]] = 0
    model.intermediate_mask[torch.randperm(model.intermediate_mask.shape[0])[:int(model.intermediate_mask.shape[0] * 0.6)]] = 0
    model.hidden_mask[torch.randperm(model.hidden_mask.shape[0])[:int(model.hidden_mask.shape[0] * 0.2)]] = 0
