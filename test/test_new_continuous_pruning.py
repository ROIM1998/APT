...
def main():
    ...
    pruned_model.head_mask = torch.load(os.path.join('output/debug_running_fisher_dynamic/pre_pruning_model_step1423', 'head_mask.pt'))
    with torch.no_grad():
        pruned_model.head_mask = torch.split(pruned_model.head_mask, [layer.attention.self.num_attention_heads for layer in model.bert.encoder.layer], dim=0)
    pruned_model.intermediate_mask = torch.load(os.path.join('output/debug_running_fisher_dynamic/pre_pruning_model_step1423', 'intermediate_mask.pt'))
    with torch.no_grad():
        pruned_model.intermediate_mask = torch.split(pruned_model.intermediate_mask, [layer.intermediate.dense.out_features for layer in model.bert.encoder.layer], dim=0)
    pruned_model.hidden_mask = torch.load(os.path.join('output/debug_running_fisher_dynamic/pre_pruning_model_step1423', 'hidden_mask.pt'))
    ...
    original_model = deepcopy(pruned_model)
    pruned_model.prune_model_with_masks()
    
    for i in range(12):
        print(pruned_model.bert.encoder.layer[i].intermediate.dense.lora_B.shape)