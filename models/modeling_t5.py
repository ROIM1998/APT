...
class AdaPT5LayerNorm(T5LayerNorm):
...
class AdaPT5ForConditionalGeneration(T5ForConditionalGeneration):
    def __init__(self, config):
        ...
        self.encoder = AdaPT5Stack(encoder_config, self.shared)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = AdaPT5Stack(decoder_config, self.shared)

        self.post_init()
        config.pruned_heads = pruned_heads
        self.config.pruned_heads = pruned_heads
        if getattr(self.config, 'adap_pruned_heads', None) is None:
    ...
    def print_lora_info_by_layer(self):
        ...
        for i in range(self.config.num_layers):
            print("Layer:", i)
            layer: AdaPT5Block = self.encoder.block[i]
        ...
        for i in range(self.config.num_decoder_layers):
            print("Layer:", i)
            layer: AdaPT5Block = self.decoder.block[i]
...
class AdaPT5DenseActDense(T5DenseActDense):
    def __init__(self, config):
        super().__init__(config)
...
class AdaPT5DenseGatedActDense(T5DenseGatedActDense):
    def __init__(self, config):
        super().__init__(config)
        self.block_retained_indices = None
        self.hidden_retained_indices = None
...
class AdaPT5LayerFF(T5LayerFF):
    def __init__(self, config):
        super().__init__(config)
        self.layer_norm = AdaPT5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        if config.feed_forward_proj == "relu":
            self.DenseReluDense = AdaPT5DenseActDense(config)
        elif config.feed_forward_proj == "gated-gelu":
            self.DenseReluDense = AdaPT5DenseGatedActDense(config)
        else:
            raise ValueError(
                f"{self.config.feed_forward_proj} is not supported. Choose between `relu` and `gated-gelu`"
            )
        # LayerNorm and Dropout layers keep the same as the original T5 model
...
class AdaPT5Attention(T5Attention):
...
class AdaPT5LayerSelfAttention(T5LayerSelfAttention):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__(config, has_relative_attention_bias)
        self.SelfAttention = AdaPT5Attention(config, has_relative_attention_bias)
        self.layer_norm = AdaPT5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        # Dropout layers keep the same as the original T5 model
...
class AdaPT5LayerCrossAttention(T5LayerCrossAttention):
    def __init__(self, config):
        super().__init__(config)
        self.EncDecAttention = AdaPT5Attention(config, has_relative_attention_bias=False)
        self.layer_norm = AdaPT5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        #Dropout layers keep the same as the original T5 model
...
class AdaPT5Block(T5Block):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__(config, has_relative_attention_bias)
        self.layer = nn.ModuleList([
            AdaPT5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias),
        ])            
        if self.is_decoder:
            self.layer.append(AdaPT5LayerCrossAttention(config))
        self.layer.append(AdaPT5LayerFF(config))
...
class AdaPT5Stack(T5Stack):
    def __init__(self, config, embed_tokens=None):
        super().__init__(config, embed_tokens)
        self.block = nn.ModuleList(
            [AdaPT5Block(config, has_relative_attention_bias=bool(i == 0)) for i in range(config.num_layers)]
        )
        self.final_layer_norm = AdaPT5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.retained_indices = None
        self.init_weights()
