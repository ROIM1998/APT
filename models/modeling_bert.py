...
from transformers.models.bert.modeling_bert import (
    ...
    BertSelfAttention, BertSelfOutput, BertIntermediate, BertPooler)
from utils.minus_utils import prune_layer, prune_layer_norm, detect_no_zero, _mask_fine_to_coarse
...
class CoFiLayerNorm(torch.nn.LayerNorm):
    ...
    def forward(self, input, hidden_z=None, use_teacher=False):
        ...
        elif self.retained_indices is not None and not use_teacher:
            compressed_weight = self.weight[self.retained_indices]
            compressed_bias = self.bias[self.retained_indices]
            normalized_shape = len(self.retained_indices)
            output = F.layer_norm(
                input, [normalized_shape], compressed_weight, compressed_bias, self.eps)
        else:
            output = F.layer_norm(
...
def reset_masks(model):
    ...
    model.head_mask = [
        torch.ones(model.bert.encoder.layer[i].attention.self.query.weight.shape[0] // attn_head_size, device=model.device)
        if model.bert.encoder.layer[i].attention.self.query is not None else None
        for i in range(model.config.num_hidden_layers)
    ]
    model.intermediate_mask = [
        torch.ones(model.bert.encoder.layer[i].intermediate.dense.weight.shape[0], device=model.device)
        if model.bert.encoder.layer[i].intermediate.dense is not None else None
        for i in range(model.config.num_hidden_layers)
    ]
    model.hidden_mask = torch.ones(model.config.hidden_size, device=model.device)
    model.head_mask = torch.cat(model.head_mask)
    model.intermediate_mask = torch.cat(model.intermediate_mask)
...
def resize_intermediate(model, kept_intermediate_dims: Dict[int, List[int]]):
    ...
    for layer in kept_intermediate_dims:
        if len(kept_intermediate_dims[layer]) == 0:
            ...
            bert.encoder.layer[layer].intermediate.dense = prune_layer(bert.encoder.layer[layer].intermediate.dense, index=torch.LongTensor(kept_intermediate_dims[layer]).to(device), dim=0)
            bert.encoder.layer[layer].output.dense = prune_layer(bert.encoder.layer[layer].output.dense, index=torch.LongTensor(kept_intermediate_dims[layer]).to(device), dim=1)
...
def named_masks(model, head_mask=None, intermediate_mask=None):
    ...
    mask_dict = {}
    for i in range(model.config.num_hidden_layers):
        query_name = "bert.encoder.layer.%d.attention.self.query" % i
        key_name = "bert.encoder.layer.%d.attention.self.key" % i
        value_name = "bert.encoder.layer.%d.attention.self.value" % i
        attn_output_name = "bert.encoder.layer.%d.attention.output.dense" % i
        up_name = "bert.encoder.layer.%d.intermediate.dense" % i
        down_name = "bert.encoder.layer.%d.output.dense" % i
...
def print_lora_info_by_layer(model):
    ...
    for i in range(model.config.num_hidden_layers):
        print("Layer:", i)
        layer: CoFiBertLayer = model.bert.encoder.layer[i]
...
def split_mask_or_score(model, head_mask: torch.Tensor, intermediate_mask: torch.Tensor) -> Tuple[List[torch.Tensor]]:
    backbone: CoFiBertModel = model.bert if hasattr(model, 'bert') else model
...
def prune_model_with_masks(model, continual_pruning=True):
    ...
    if isinstance(hidden_mask, torch.Tensor) and hidden_mask.numel() != hidden_size:
        ...
        model.bert.embeddings.word_embeddings.weight = torch.nn.parameter.Parameter(
            model.bert.embeddings.word_embeddings.weight.index_select(1, index).detach().clone())
        model.bert.embeddings.word_embeddings.embedding_dim = index.shape[0]
        model.bert.embeddings.position_embeddings.weight = torch.nn.parameter.Parameter(
            model.bert.embeddings.position_embeddings.weight.index_select(1, index).detach().clone())
        model.bert.embeddings.position_embeddings.embedding_dim = index.shape[0]
        model.bert.embeddings.token_type_embeddings.weight = torch.nn.parameter.Parameter(
            model.bert.embeddings.token_type_embeddings.weight.index_select(1, index).detach().clone())
        model.bert.embeddings.token_type_embeddings.embedding_dim = index.shape[0]
        prune_layer_norm(model.bert.embeddings.LayerNorm, index)

        for layer in range(0, 12):
            print("Pruning layer:", layer)
            if model.bert.encoder.layer[layer].attention.self.query is not None:
                model.bert.encoder.layer[layer].attention.self.query = \
                    prune_layer(model.bert.encoder.layer[layer].attention.self.query , index, dim=1)
                model.bert.encoder.layer[layer].attention.self.key = \
                    prune_layer(model.bert.encoder.layer[layer].attention.self.key , index, dim=1)
            if model.bert.encoder.layer[layer].attention.self.value is not None:
                model.bert.encoder.layer[layer].attention.self.value = \
                    prune_layer(model.bert.encoder.layer[layer].attention.self.value , index, dim=1)
                model.bert.encoder.layer[layer].attention.output.dense = \
                    prune_layer(model.bert.encoder.layer[layer].attention.output.dense , index, dim=0)
                prune_layer_norm(model.bert.encoder.layer[layer].attention.output.LayerNorm, index)
            if model.bert.encoder.layer[layer].intermediate.dense is not None:
                model.bert.encoder.layer[layer].intermediate.dense = \
                    prune_layer( model.bert.encoder.layer[layer].intermediate.dense, index, dim=1)
                model.bert.encoder.layer[layer].output.dense = \
                    prune_layer( model.bert.encoder.layer[layer].output.dense, index, dim=0)
                prune_layer_norm(model.bert.encoder.layer[layer].output.LayerNorm, index)
        ...
        if hasattr(model.bert.pooler, "dense"):
            model.bert.pooler.dense = prune_layer(model.bert.pooler.dense, index, dim=1)
    ...
    model.hidden_mask = hidden_mask[model.hidden_mask.nonzero().squeeze()].detach().contiguous().flatten() if hidden_mask is not None else None
...
def virtual_prune(model):
    ...
    model.bert.embeddings.retained_indices = hidden_retained_indices
    model.bert.embeddings.LayerNorm.retained_indices = hidden_retained_indices
    for layer in range(model.config.num_hidden_layers):
        ...
        model.bert.encoder.layer[layer].attention.self.num_teacher_attention_heads = model.bert.encoder.layer[layer].attention.self.num_attention_heads
        model.bert.encoder.layer[layer].attention.self.num_attention_heads = num_retained_heads 
        model.bert.encoder.layer[layer].attention.self.block_retained_indices = block_retained_indices
        model.bert.encoder.layer[layer].attention.self.hidden_retained_indices = hidden_retained_indices
        model.bert.encoder.layer[layer].attention.output.block_retained_indices = block_retained_indices
        model.bert.encoder.layer[layer].attention.output.hidden_retained_indices = hidden_retained_indices
        model.bert.encoder.layer[layer].attention.output.LayerNorm.retained_indices = hidden_retained_indices
        
        block_retained_indices = intermediate_mask[layer].nonzero().squeeze()
        model.bert.encoder.layer[layer].intermediate.block_retained_indices = block_retained_indices
        model.bert.encoder.layer[layer].intermediate.hidden_retained_indices = hidden_retained_indices
        model.bert.encoder.layer[layer].output.block_retained_indices = block_retained_indices
        model.bert.encoder.layer[layer].output.hidden_retained_indices = hidden_retained_indices
        model.bert.encoder.layer[layer].output.LayerNorm.retained_indices = hidden_retained_indices
    
    if hasattr(model.bert, "pooler"):
        model.bert.pooler.retained_indices = hidden_retained_indices
...
def virtual_prune_restore(model):
    ...
    for layer in range(model.config.num_hidden_layers):
        model.bert.encoder.layer[layer].attention.self.block_retained_indices = None
        model.bert.encoder.layer[layer].attention.self.hidden_retained_indices = None
        model.bert.encoder.layer[layer].attention.self.num_attention_heads = model.bert.encoder.layer[layer].attention.self.num_teacher_attention_heads
        model.bert.encoder.layer[layer].attention.output.block_retained_indices = None
        model.bert.encoder.layer[layer].attention.output.hidden_retained_indices = None
        model.bert.encoder.layer[layer].attention.output.LayerNorm.retained_indices = None
        model.bert.encoder.layer[layer].intermediate.block_retained_indices = None
        model.bert.encoder.layer[layer].intermediate.hidden_retained_indices = None
        model.bert.encoder.layer[layer].output.block_retained_indices = None
        model.bert.encoder.layer[layer].output.hidden_retained_indices = None
        model.bert.encoder.layer[layer].output.LayerNorm.retained_indices = None
        
    model.bert.embeddings.retained_indices = None
    model.bert.embeddings.LayerNorm.retained_indices = None
    if hasattr(model.bert, "pooler"):
        model.bert.pooler.retained_indices = None
...
def restore_model_from_history(model):
    ...
    q_template = 'bert.encoder.layer.%d.attention.self.query'
    k_template = 'bert.encoder.layer.%d.attention.self.key'
    v_template = 'bert.encoder.layer.%d.attention.self.value'
    o_template = 'bert.encoder.layer.%d.attention.output.dense'
    up_template = 'bert.encoder.layer.%d.intermediate.dense'
    down_template = 'bert.encoder.layer.%d.output.dense'
    ...
    for layer in range(model.config.num_hidden_layers):
        ...
        model.bert.encoder.layer[layer].attention.self.num_attention_heads = model.config.num_attention_heads
        model.bert.encoder.layer[layer].attention.self.num_teacher_attention_heads = model.config.num_attention_heads
        model.bert.encoder.layer[layer].attention.pruned_heads = set()
        model.bert.encoder.layer[layer].attention.self.query.in_features = model.config.hidden_size
        model.bert.encoder.layer[layer].attention.self.query.out_features = model.config.hidden_size
        model.bert.encoder.layer[layer].attention.self.key.in_features = model.config.hidden_size
        model.bert.encoder.layer[layer].attention.self.key.out_features = model.config.hidden_size
        model.bert.encoder.layer[layer].attention.self.value.in_features = model.config.hidden_size
        model.bert.encoder.layer[layer].attention.self.value.out_features = model.config.hidden_size
        model.bert.encoder.layer[layer].attention.output.dense.in_features = model.config.hidden_size
        model.bert.encoder.layer[layer].attention.output.dense.out_features = model.config.hidden_size
        model.bert.encoder.layer[layer].intermediate.dense.in_features = model.config.hidden_size
        model.bert.encoder.layer[layer].intermediate.dense.out_features = model.config.intermediate_size
        model.bert.encoder.layer[layer].output.dense.in_features = model.config.intermediate_size
        model.bert.encoder.layer[layer].output.dense.out_features = model.config.hidden_size
...
class CoFiBertForSequenceClassification(BertForSequenceClassification):
    ...
    def _mask_fine_to_coarse(self, mask):
        return _mask_fine_to_coarse(self, mask)

    def resize_intermediate(self, kept_intermediate_dims: Dict[int, List[int]]):
        resize_intermediate(self, kept_intermediate_dims)
    ...
    def forward(
        ...
        if use_cross_masked_states:
            masked_pooled_output = outputs[-1]
            masked_pooled_output = self.dropout(masked_pooled_output)
        else:
            masked_pooled_output = None

        
        logits = self.classifier(pooled_output) #! [32, 3]
        if use_cross_masked_states:
            if isinstance(self.classifier, lora.DistillLinear):
                masked_logits = self.classifier(masked_pooled_output, use_teacher=False)
            else:
                masked_logits = self.classifier(masked_pooled_output)
        else:
            masked_logits = None
...
class AdaPBertForQuestionAnswering(BertForQuestionAnswering):
    def __init__(self, config):
        super().__init__(config)
        self.bert = CoFiBertModel(config)
    ...
    def forward(
        ...
        if use_cross_masked_states:
            masked_sequence_output = outputs[-2]
        ...
        return NewQuestionAnsweringModelOutput(
            ...
            masked_states=None, # TODO: add masked states when using in-layer distillation
        )
...
class CoFiBertPooler(BertPooler):
    def __init__(self, config):
        super().__init__(config)
        self.retained_indices = None

    def forward(self, hidden_states: torch.Tensor, use_teacher: bool = False) -> torch.Tensor:
        # We "pool" the model by simply taking the hidden state corresponding
        # to the first token.
        first_token_tensor = hidden_states[:, 0]
        if isinstance(self.dense, lora.PruningLinear):
            pooled_output = self.dense(first_token_tensor, use_teacher=use_teacher, in_retained_indices=self.retained_indices)
        elif use_teacher:
            pooled_output = self.dense(first_token_tensor)
        else:
            selected_weight, selected_bias = select_wandb(self.dense.weight, self.dense.bias, self.retained_indices)
            pooled_output = F.linear(first_token_tensor, selected_weight, selected_bias)
...
class CoFiBertModel(BertModel):
    def __init__(self, config):
        ...
        self.pooler = CoFiBertPooler(config)
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds, hidden_z=hidden_z, use_teacher=use_teacher
        )
        ...
        if self.pooler is not None:
            ...
            if output_masked_states and use_cross_masked_states:
                masked_pooled_output = self.pooler(encoder_outputs[-1][-1]) # Get the last layer's masked hidden states
            else:
                masked_pooled_output = None
        else:
            pooled_output = None
        ...
        if not return_dict:
            if masked_pooled_output is not None:
                return (sequence_output, pooled_output) + encoder_outputs[1:] + (encoder_outputs[-1][-1], masked_pooled_output,) # appended masked last layer hidden states and masked-pooled output
            else:
        ...
        return NewBaseModelOutputWithPooling(
            ...
            masked_pooler_output=masked_pooled_output,
        )
...
class CoFiBertLayer(BertLayer):
    def __init__(self, config):
        ...
        self.intermediate = CoFiBertIntermediate(config)
        self.config = config
...
class CoFiBertIntermediate(BertIntermediate):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(self, hidden_states, input_tensor, mlp_z, hidden_z=None, inference=False, use_teacher=False):
        ...
        if not inference and hidden_states.sum().eq(0).item():
            ...
            hidden_states = self.LayerNorm(
                hidden_states + input_tensor, hidden_z, use_teacher=use_teacher)
            if hidden_z is not None:
                hidden_states = hidden_states.mul(hidden_z)
        return hidden_states